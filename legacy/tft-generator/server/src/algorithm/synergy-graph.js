...
function buildOneTeam(graph, teamSize, startChamps, context, temperature, rng) {
  ...
  const seeds = startChamps.filter(api => nodes[api] && !lockedSet.has(api));
...
export function findTeams(graph, options = {}) {
  ...
  return diversifyResults(results, maxResults, traitBreakpoints, emblems);
}
