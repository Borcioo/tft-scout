 ...
 */
export function teamRoleBalance(champions) {
  let frontline = 0, dps = 0, fighter = 0;
  for (const c of champions) {
    const cat = ROLE_CATEGORY[c.role] || 'fighter';
 ...
 */
export function roleBalancePenalty(champions) {
...
function starPowerFallback(cost, level) {
  ...
  const starPowers = expectedStarPower[level] || expectedStarPower[8];
  return (baseStat(cost) * starPowers[costIdx]) / 1.5;
}

function baseStat(cost) { return cost + 1; }

export function championScore(champion, ctx, level = 8) {
...
export function traitScore(trait, ctx) {
...
export function affinityBonus(champion, activeTraitApis, ctx) {
  const lookupApi = champion.baseApiName || champion.apiName;
 ...
 */
function dominantTraitDampen(activeTraits, ctx) {
  let bestAvg = 8;
 ...
 */
export function orphanPenalty(champion, activeTraitApis) {
...
export function companionBonus(team, ctx) {
  if (!ctx.companions) return 0;

  let bonus = 0;
  const teamApis = new Set(team.champions.map(c => c.baseApiName || c.apiName));
  ...
  for (const champApi of teamApis) {
    const companionList = ctx.companions[champApi];
    ...
    for (const comp of companionList) {
      ...
      const pairKey = [champApi, comp.companionApiName].sort().join('+');
...
export function teamScore(team, ctx) {
  let score = 0;
  const { level = 8 } = team;
...
export function teamScoreBreakdown(team, ctx) {
  const { level = 8 } = team;
  const breakdown = { champions: 0, traits: 0, affinity: 0, companions: 0, synergy: 0, balance: 0, total: 0 };
